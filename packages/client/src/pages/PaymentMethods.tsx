...
export default function PaymentMethods() {
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      const db = getDB();
      await db.exec({ sql: "UPDATE payment_methods SET name=?, updated_at=datetime('now') WHERE id=?", bind: [name, id] });
      setEditing(null);
      await load();
    } catch {}
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      const db = getDB();
      await db.exec({ sql: "UPDATE payment_methods SET deleted_at = datetime('now'), updated_at = datetime('now') WHERE id=?", bind: [deleteId] });
      setDeleteId(null);
      await load();
    } catch {}
  };
